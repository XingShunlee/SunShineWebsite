    ...
    public class MyGameCenterController : Controller
    {
        ...
        public JsonResult GetGameListInfo(int page = 1, int rows = 10, int type = -1, string name = "")
        {
            ...
            if (type >= 0)
            {
                ...
                if (type == 0)
                {
                    query1 = (from c in GamelistManager.GetDbSet()
                              ...
                              where loginuser.UserGuid == d.UserGuid
                              select c).ToList();
                }
                else
                {
                    query1 = (from c in GamelistManager.GetDbSet()
                              from d in GameManager.GetDbSet()
                              where c.Gametype == d.Gametype && loginuser.UserGuid == d.UserGuid && c.Gametype == type
                              select c).ToList();
                }
            ...
            else
            {
                ...
                query1 = (from c in GamelistManager.GetDbSet()
                          from d in GameManager.GetDbSet()
                          where c.ItemName.Contains(name) && c.ItemID == d.GameID &&
        ...
        {
            if (ItemId <= 0)
                return RedirectToRoute(new { Controller = "MyGameCenter", Action = "Index" });
            GameListManager GamelistManager = new GameListManager(DbContext);
            ...
            if (query != null)
            {
                ...
                var types = _noteTypeRepository.List();
                ViewBag.GameTypes = types.Select(r => new SelectListItem { Text = r.Name, Value = r.GameId.ToString() });
                return View("EditGameItem", query);
            }
            else
            {
                return RedirectToRoute(new { Controller = "MyGameCenter", Action = "Index" });
        ...
        [HttpPost]
        [LoginStateRequiredAttribute]
        public JsonResult CancelMyGameItem(int gid)
        {
            int errorCode = 0;
            string errMsg = "操作失败！";
        ...
        public JsonResult GetMyGameListInfo(int page = 1, int rows = 10, int type = 1)
        {
            ...
            if (type >= 0)
            {
                ...
                if (type == 0)
                {
                    query1 = (from c in GamelistManager.GetDbSet()
                              from d in GameManager.GetDbSet()
                              where c.ItemID == d.GameID && loginuser.UserGuid == d.UserGuid
                              select c).ToList();
                }
                else
                {
                    query1 = (from c in GamelistManager.GetDbSet()
                              from d in GameManager.GetDbSet()
                              where c.ItemID == d.GameID && c.Gametype == d.Gametype &&
                              loginuser.UserGuid == d.UserGuid && c.Gametype == type
                              select c).ToList();
                }
